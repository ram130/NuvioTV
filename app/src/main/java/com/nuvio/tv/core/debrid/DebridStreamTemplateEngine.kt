...
class DebridStreamTemplateEngine @Inject constructor() {
    ...
    private fun evaluateSingleCondition(value: Any?, ops: List<String>): Boolean {
        ...
        ops.forEach { op ->
            when {
                ...
                op.startsWith("~=") -> {
                    val target = op.drop(2).trim()
                    result = containsText(value, target)
                    hasResult = true
                }
                op.startsWith("~") -> {
                    val target = op.drop(1).trim()
                    result = containsText(value, target)
                    hasResult = true
                }
                op.startsWith("=") -> {
                    val target = op.drop(1).trim()
                    result = equalsText(value, target)
                    hasResult = true
                }
                op.startsWith(">=") -> {
    ...
    private fun String.isFieldPath(): Boolean {
        return startsWith("stream.") || startsWith("service.") || startsWith("addon.")
    }
    ...
    private fun equalsText(value: Any?, target: String): Boolean {
        ...
        return when (value) {
            is Iterable<*> -> value.any { valueToText(it).trim().equals(normalized, ignoreCase = true) }
            else -> valueToText(value).trim().equals(normalized, ignoreCase = true)
        }
    }
    ...
    private fun containsText(value: Any?, target: String): Boolean {
        val normalized = target.trim()
        return when (value) {
            is Iterable<*> -> value.any { valueToText(it).contains(normalized, ignoreCase = true) }
            else -> valueToText(value).contains(normalized, ignoreCase = true)
        }
    }
